/*
Google only supports EMCA 2015 standards, so static variables cannot be declared normally.
Additionally, the need to import or export functions is unneccessary as GAS does this
automagically. 
*/
...
class Tiers { //This class has properties and methods relating to tiers and their number equivalents
  ...
  static tierCalculator( row ) { //row param refers to the row number someone sits on.
      const tierCopier = getSheet("Training log").getRange( row, 4, 1, 4 ).getDisplayValues(); //Obtains 2D array of tier letters, [1] is useless
...
Tiers.Tiers = ["UNRATED", "F-", "F", "F+", "D-", "D", "D+", "C-", "C", "C+",
  "B-", "B", "B+", "A-", "A", "A+", "S-", "S", "S+"];
...
static isDischarged(row) {
  const searchRange = sheetGet("Management").getRange(row + 1, 5).getDisplayValue(); //Checks column 5 (Status) at param row + 1 (management rows are offset by 1)
...
class RowBuilder { //Properties and methods to actually build stuff on the sheet.
  ...
  static setStatus(row) { //takes personnel row number as param
      ...
      const paste = function(val){ 
          getSheet("Personnel").getRange(row, 8).setValue(val); //column 8 is where personnel status column is
      };
  ...
  static setRank(row, val) {
      getSheet("Personnel").getRange(row, 6).setValue(val); //column 6 is where rank column is
  };
}